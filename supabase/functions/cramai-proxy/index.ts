  ...
  switch (endpoint) {
    ...
    case '/api/questions/grade': {
      ...
      return await callAIJSON({
        ...
        model: MODEL_STRUCTURED,
        system:
          'You are a strict but fair exam grader marking like a real examiner against the official mark scheme.\n' +
          '- For multiple-choice/true-false: isCorrect only if the user answer matches the correct option exactly (case-insensitive). marksAvailable = 1, marksAwarded = 1 if correct else 0. markBreakdown can be empty or a single line.\n' +
          '- For short-answer and essay: act as a meticulous examiner. Use the question\'s markScheme.points if present (each has marks); otherwise infer fair mark points from the correctAnswer (typically 2–6 points for short-answer, 6–20 for essay). For EVERY mark-scheme point you MUST decide marksAwarded out of marksAvailable for that point (allow half marks where appropriate) and write a short examiner note saying exactly why those marks were or were not awarded, quoting/paraphrasing the student where relevant. Reward correct content, valid synonyms, and equivalent reasoning. Penalise vague claims, missing key terms, factual errors, missing structure, missing evidence, lack of command-word response (e.g. "evaluate", "explain", "compare").\n' +
          '- For each answer also produce: examinerFeedback (2–4 sentences, examiner voice, what was strong and what cost marks), improvementTips (2–4 specific, actionable tips), missingPoints (key points from the mark scheme the student did not hit).\n' +
          '- marksAwarded for the whole answer = sum of marksAwarded across markBreakdown, capped at marksAvailable. isCorrect = marksAwarded >= 0.7 * marksAvailable for written answers; for MCQ/true-false the strict match rule above.\n' +
          '- score = sum of marksAwarded across all answers. totalMarksAvailable = sum of marksAvailable. percentage = round(totalMarksAwarded / totalMarksAvailable * 100). totalQuestions = number of questions. weakTopics = topics where the student lost most marks. recommendations = 3–5 concrete next steps.\n' +
          'Return ONLY valid JSON matching the schema. No prose.' + HTML_FORMAT_RULES,
        user: `Questions (each may include a markScheme with point-level marks):\n${truncate(questions)}\n\nUser answers (keyed by question id):\n${truncate(userAnswers)}\n\nReturn JSON:\n{\n  "score": number,\n  "totalQuestions": number,\n  "totalMarksAwarded": number,\n  "totalMarksAvailable": number,\n  "percentage": number,\n  "answers": [{\n    "questionId": string,\n    "isCorrect": boolean,\n    "userAnswer": string,\n    "correctAnswer": string,\n    "explanation": string,\n    "marksAwarded": number,\n    "marksAvailable": number,\n    "markBreakdown": [{"point": string, "marksAwarded": number, "marksAvailable": number, "note": string}],\n    "examinerFeedback": string,\n    "improvementTips": string[],\n    "missingPoints": string[]\n  }],\n  "weakTopics": string[],\n  "recommendations": string[]\n}`,
        maxTokens: 5000,
      });
    }
    ...
    case '/api/notes/generate': {
      ...
      const avoidBlock = avoidClaims.length
        ? `\n\nSTRICT ACCURACY GUARD — a previous draft contained claims not supported by the source. You MUST NOT include, rephrase, or imply ANY of the following claims (or anything semantically equivalent). If you cannot support a point directly from the SOURCE, omit it entirely.\nFORBIDDEN CLAIMS:\n${avoidClaims.map((c, i) => `${i + 1}. ${c}`).join('\n')}\n\nEvery statement in your output must be directly grounded in the provided material. Prefer omitting a point over guessing.`
        : '';
      return await callAIJSON({
        apiKey,
        model: MODEL_STRUCTURED,
        system:
          'You write study notes that look and read like a clean handwritten exercise book / printed textbook page — formatted so a student can copy them straight into their book word-for-word. STRICTLY use the provided material. Requirements: (1) Title is a proper topic title (no first-person, no AI phrasing). (2) Overview is 2-4 sentences of factual introduction, textbook tone — third person, no "we" or "let\'s". (3) Each section heading is a short noun phrase like a subtopic heading you would underline in a notebook. (4) Section "body" is 2-5 sentences of crisp expository prose written like a textbook paragraph — definitions, statements of fact, cause/effect. No conversational filler, no questions to the reader, no "imagine that…", no emojis. (5) "bullets" are short, parallel-structured notes (under 18 words each) that a student would jot down — key facts, definitions, steps, formulas, comparisons. Use <strong> for key terms. (6) "examples" are concrete worked examples or named real-world cases, each 1-2 sentences. (7) "keyTakeaways" are 4-6 concise revision-ready statements. Adapt depth to the exam level/board. ADHD profile may shorten paragraphs and add more bullets, but the TEXTBOOK STYLE must be preserved — never break into chatty tutor voice in the visible notes. Every claim must be directly supported by the SOURCE material — do not invent facts, figures, dates, names, or examples that are not in the source. When in doubt, omit. Return ONLY valid JSON.' + HTML_FORMAT_RULES + syllabusSystem(body) + adhdSystem(body),
    ...
    case '/api/notes/spoken-script': {
      ...
      return await callAIJSON({
        ...
        model: MODEL_CHAT,
        system:
          'You convert structured study notes into a warm, in-depth spoken tutor script. The VISIBLE notes are textbook-style; your job is to bring them to life out loud so the learner deeply understands every point. RULES: (1) Open by reading the overview as context, then briefly orient the listener. (2) Go through every section in order: announce the heading, restate the main idea in plain words, then for EACH bullet and example explain it thoroughly — what it means, why it matters, how it works, common misconceptions, and a fresh analogy or worked example where useful. Go beyond what the bullet literally says; add the reasoning a good tutor would add. (3) Use natural conversational transitions ("Okay, next up…", "Here\'s the intuition…", "Think of it this way…"). Be encouraging but never childish. (4) Pronounce formulas in words (e.g. "F equals m a"). (5) End by walking through the key takeaways and tying them back to the overview. (6) Output PLAIN spoken English ONLY — no markdown, no HTML tags, no bullet symbols, no headings syntax, no asterisks, no stage directions. Return ONLY valid JSON.',
        user: `Subject: ${subject}\n\nNotes JSON:\n${JSON.stringify(notes).slice(0, 12000)}\n\nReturn JSON: { "script": string (the complete spoken script, aim for 1200-2500 words depending on notes length, plain spoken English, explain each point in depth) }`,
        maxTokens: 4000,
      });
    }
